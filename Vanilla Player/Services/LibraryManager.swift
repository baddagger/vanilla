...
class LibraryManager: ObservableObject {
    ...
    @discardableResult
    private func scanSource(_ source: Source, mode: ScanMode) async -> [Track] {
        let (newTracks, allURLs) = self.collectTracks(from: source, existingTracks: self.tracks)
    ...
    private func loadLibrary() {
        ...
        do {
            ...
            self.sources = libraryData.sources
            self.tracks = libraryData.tracks
